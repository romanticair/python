...
录入字段更好）；对于字节数大于len(line)的读取请求。分行会出现问题；也可向GuiInput里
增加__iter__或者__next__以像文件一样支持迭代，但会出现过多的弹窗。
####################################################################################
"""
from tkinter import *
from tkinter.simpledialog import askstring
from tkinter.scrolledtext import ScrolledText           # or Gui.Tour.scrolledText
...
class GuiInput:
    ...
    def inputLine(self):
        line = askstring('GuiInput', 'Enter input line + <cflr> (cancel=eof)')
        if line is None:
            return ''               # 针对各行弹出对话框
        else:                       # 取消按钮表示文件末尾
            return line + '\n'      # 否则，添加行结束的标记
...
def redirectedGuiShellCmd(command):
    ...
    def reader(input, output):                     # 显示一个shell命令的标准输出
        while True:
            line = input.readline()                  # 在新的弹出式文本框组件中
            if not line:                            # 调用readline时可能阻塞
                break
            output.write(line)
    reader(input, output)
...
if __name__ == '__main__':
    ...
    def makeLower(input, output):                 # 使用显示文件
        while True:
            line = input.readline()
            if not line:
                break
