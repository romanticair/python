...
class SumGrid(Frame):
    def __init__(self, parent=None, numrow=5, numcol=5):
        ...
        self.makeWidgets(numrow, numcol)                   # else only useable one way
    ...
    def makeWidgets(self, numrow, numcol):
        ...
        # Quitter(self).grid(row=0, column=4)           # fails: Quiiter(self).pack()
    ...
    def onSum(self):
        ...
        for i in range(self.numcol):
            for j in range(self.numrow):
                tots[i] += eval(self.rows[j][i].get())  # sum column
        for i in range(self.numcol):
            self.sums[i].config(text=str(tots[i]))      # display in GUI
