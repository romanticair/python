...
dict, but html.escape to put them in HTML hidden fields;  some tools here may be
...
def formatlink(cgiurl, parmdict):
    ...
    note that URL escapes are different from HTML (html.escape)
    """
...
def messagearea(headers, text, extra=''):               # extra for readonly
    ...
    for hdr in ('From', 'To', 'Cc', 'Subject'):
        ...
        val = html.escape(dechdr, quote=True)
    ...
    print('%s\n</textarea></table>' % (html.escape(text) or '?'))  # if has </>s
...
def viewpage(msgnum, headers, text, form, parts=[]):
    ...
    in HTML here, so we use html.escape; this usually sends nonprintable
